...
const BOT = 'fritz';
function withTelemetry(handler) { return function(...args) { return context.run({command:'/unknown',ids:[]},()=>handler.apply(this,args)); }; }
...
function eventFor(url,payload,data,started,id,error){
  ...
  let text=[data?.output_text,data?.text,data?.choices?.[0]?.message?.content].find(value=>typeof value==='string'&&value.trim()) || (Array.isArray(data?.output)?data.output:[]).flatMap(x=>(Array.isArray(x.content)?x.content:[]).map(y=>y.text||y.refusal||'')).filter(value=>typeof value==='string'&&value).join('\n');
  // Resolve the selector's index against the actual candidates sent in this call.
  if(payload.text?.format?.name==='shae_candidate_selection'){
    try{
      const selected=JSON.parse(text);
      const input=(payload.input||[]).flatMap(item=>item.content||[]).filter(item=>item.type==='input_text').map(item=>item.text).join('\n');
      const candidates=input.split('Numbered candidates:\n').at(-1);
      const matches=[...candidates.matchAll(/^(\d+): ([\s\S]*?)(?=^\d+: |$(?![\s\S]))/gm)];
      const match=matches.find(item=>Number(item[1])===selected.winner);
      if(Number.isInteger(selected.winner)&&match) text=JSON.stringify({selected_line:match[2].trim(),candidate_index:selected.winner});
    }catch{}
  }
...
async function discordFetch(url,options){
 const ctx=context.getStore();const target=new URL(String(url));
 if(options?.method==='POST'&&/\/webhooks\/[^/]+\/[^/]+$/.test(target.pathname))target.searchParams.set('wait','true');
 const response=await originalFetch(target.toString(),options);
 if(!response.ok||!ctx?.ids?.length)return response;
 try{
  const raw=requestPayload(options?.body);const sent=raw.payload_json?JSON.parse(raw.payload_json):raw;
  const data=await response.clone().json().catch(()=>({}));
  const output=typeof data.content==='string'?data.content:sent.content;
  const imageUrls=(data.attachments||[]).filter(a=>a.content_type?.startsWith('image/')||/\.(png|jpe?g|webp|gif)(?:\?|$)/i.test(a.url||'')).map(a=>a.url).filter(u=>typeof u==='string'&&u.startsWith('https://')).slice(0,10);
  // Skip placeholder edits/deferred acknowledgements, not final replies.
  if((typeof output==='string'&&output.trim()&&!/^[|\s]+$/.test(output))||imageUrls.length){
   for(let i=0;i<ctx.ids.length;i+=200)await deliver('result',{ids:ctx.ids.slice(i,i+200),bot:BOT,output:description(output||''),imageUrls});
  }
 }catch{}
 return response;
}
async function monitoredFetch(url,options){
 if(process.env.BOTMETER_URL&&process.env.BOTMETER_INGEST_KEY&&/^https:\/\/(?:discord.com|discordapp.com)\/api\/(?:v\d+\/)?(?:webhooks\/|channels\/[^/]+\/messages)/.test(String(url))&&['POST','PATCH'].includes(options?.method))return discordFetch(url,options);
  ...
  if(context.getStore())context.getStore().ids.push(id);
  let response;
